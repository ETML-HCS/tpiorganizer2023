...
} = require('./coordinationTpiVisibility')
const {
...
function isParticipantAvailable(participant, task, dateKey, period) {
  ...
  if (typeof participant.person.isAvailableOn === 'function') {
    return participant.person.isAvailableOn(new Date(`${dateKey}T08:00:00.000Z`), period, {
      numSlots: task?.siteContext?.numSlots
    })
  }
...
function isProjectLeadParticipant(participant) {
  return ['chefProjet', 'chef_projet', 'project_lead']
    .includes(compactText(participant?.role))
}

function getUnavailableParticipantsForBlocking(task, dateKey, period) {
  return (task.participants || []).filter((participant) =>
    !isProjectLeadParticipant(participant) &&
    !isParticipantAvailable(participant, task, dateKey, period)
  )
}

function getUnavailableProjectLeadCount(task, dateKey, period) {
  return (task.participants || []).filter((participant) =>
    isProjectLeadParticipant(participant) &&
    !isParticipantAvailable(participant, task, dateKey, period)
  ).length
...
function createConstraintIssue(type, description, participants = []) {
  const involvedPersons = Array.from(
    new Set(
      (Array.isArray(participants) ? participants : [])
        .map((participant) => participant?.personId || toObjectIdString(participant?.person))
        .filter(Boolean)
    )
  )
...
function normalizeConstraintIssues(issues = []) {
  const normalizedIssues = []
  const seen = new Set()

  for (const issue of Array.isArray(issues) ? issues : []) {
    const type = compactText(issue?.type) || 'automatic_constraint_override'
    ...
    normalizedIssues.push({
      type,
      description,
      involvedPersons: Array.from(
        new Set(
          (Array.isArray(issue?.involvedPersons) ? issue.involvedPersons : [])
...
function getOccupiedParticipantsOnTimeKey(state, task, timeKey) {
  const occupiedPeople = state.personOccupancyByTimeKey.get(timeKey)
  if (!occupiedPeople) {
    return []
  }

  return (task.participants || []).filter((participant) => occupiedPeople.has(participant.personId))
}

function buildFallbackConstraintIssues(task, state, timeline, placement) {
  const dateKey = placement?.dateKey
  const period = placement?.period
  const timeKey = toTimeStepKey(dateKey, period)
  const issues = []
  const unavailableParticipants = getUnavailableParticipantsForBlocking(task, dateKey, period)

  if (unavailableParticipants.length > 0) {
    issues.push(createConstraintIssue(
      'availability_override',
      `Disponibilité ignorée: ${unavailableParticipants.map((participant) => participant.personName || formatPersonName(participant.person)).filter(Boolean).join(', ')} indisponible(s) le ${dateKey} au créneau ${period}.`,
      unavailableParticipants
    ))
  }

  const overlappingParticipants = getOccupiedParticipantsOnTimeKey(state, task, timeKey)
  if (overlappingParticipants.length > 0) {
    issues.push(createConstraintIssue(
      'person_overlap',
      `Chevauchement forcé: ${overlappingParticipants.map((participant) => participant.personName || formatPersonName(participant.person)).filter(Boolean).join(', ')} déjà affecté(s) le ${dateKey} au créneau ${period}.`,
      overlappingParticipants
    ))
  }

  const consecutiveParticipants = (task.participants || []).filter((participant) =>
    exceedsConsecutiveLimit(state, timeline, participant.personId, timeKey, task?.siteContext?.maxConsecutiveTpi)
  )
  if (consecutiveParticipants.length > 0) {
    issues.push(createConstraintIssue(
      'consecutive_limit',
      `Limite de TPI consécutifs ignorée: ${consecutiveParticipants.map((participant) => participant.personName || formatPersonName(participant.person)).filter(Boolean).join(', ')} dépasserait la limite sur le créneau ${period}.`,
      consecutiveParticipants
    ))
  }

  if (!isRoomCompatibleWithTask(task?.siteContext, placement?.roomName, task)) {
    issues.push(createConstraintIssue(
      'room_class_mismatch',
      `Salle forcée malgré une incompatibilité de classe: ${placement?.roomName || 'salle inconnue'}.`
    ))
  }

  return normalizeConstraintIssues(issues)
}

function getConstraintIssuePenalty(issue) {
  switch (compactText(issue?.type)) {
    case 'person_overlap':
      return 100000
    case 'room_class_mismatch':
      return 50000
...
function getPlacementConstraintPenalty(placement) {
  return (Array.isArray(placement?.constraintIssues) ? placement.constraintIssues : [])
    .reduce((total, issue) => total + getConstraintIssuePenalty(issue), 0)
}

function compareFallbackPlacements(left, right) {
  const penaltyCompare = getPlacementConstraintPenalty(left) - getPlacementConstraintPenalty(right)
  if (penaltyCompare !== 0) {
    return penaltyCompare
  }

  const issueCountCompare = (left.constraintIssues?.length || 0) - (right.constraintIssues?.length || 0)
  if (issueCountCompare !== 0) {
    return issueCountCompare
  }
  ...
  if (left.projectLeadUnavailableCount !== right.projectLeadUnavailableCount) {
    return left.projectLeadUnavailableCount - right.projectLeadUnavailableCount
  }
  ...
  return compactText(left.roomName).localeCompare(compactText(right.roomName))
}

function enumerateFallbackPlacements(task, state, timeline) {
  const placements = []
  ...
  task.allowedDateKeys.forEach((dateKey, dateIndex) => {
    ...
    let candidateRooms = buildCandidateRooms(state, siteContext, dateKey, preferredRooms, task)

    if (candidateRooms.length === 0) {
      candidateRooms = buildCandidateRooms(state, siteContext, dateKey, preferredRooms, null)
    }
    ...
    for (let period = 1; period <= siteContext.numSlots; period += 1) {
      for (const room of candidateRooms) {
        ...
        const placement = {
          ...
          projectLeadUnavailableCount: getUnavailableProjectLeadCount(task, dateKey, period),
          proximityPenalty: computeProximityPenalty(state, task, dateKey, period)
        }

        placements.push({
          ...placement,
          constraintIssues: buildFallbackConstraintIssues(task, state, timeline, placement),
  ...
  placements.sort(compareFallbackPlacements)
  return placements
}
...
function enumerateCandidatePlacements(task, state, timeline) {
  ...
  task.allowedDateKeys.forEach((dateKey, dateIndex) => {
    ...
    for (let period = 1; period <= siteContext.numSlots; period += 1) {
      ...
      if (getUnavailableParticipantsForBlocking(task, dateKey, period).length > 0) {
        continue
      }
...
function computeForcedAssignments(tasks, state, timeline) {
  const assignments = []
  const manualRequired = []

  for (const task of Array.isArray(tasks) ? tasks : []) {
    const candidates = enumerateFallbackPlacements(task, state, timeline)

    if (candidates.length === 0) {
      manualRequired.push(createManualIssue(task, 'Aucun créneau de secours n a pu etre genere pour ce TPI.'))
      continue
    }
    ...
    assignments.push({
      task: {
        ...task,
        automaticPlanningWarnings: placement.constraintIssues
      },
      placement,
      constraintIssues: placement.constraintIssues,
      isConstraintOverride: true
    })
  }
...
function computeAutomaticAssignments(tasks = []) {
  ...
  const forcedResult = computeForcedAssignments(greedyResult.unassignedTasks, greedyResult.state, timeline)
  const combinedAssignments = [
    ...greedyResult.assignments,
    ...forcedResult.assignments
  ]
  const balancedGreedyResult = rebalanceOpenRooms(
    combinedAssignments,
    forcedResult.generatedRoomsBySiteDate
  )
  ...
  return {
    ...
    manualRequired: [
      ...manualRequired,
      ...forcedResult.manualRequired
    ],
    constraintOverrideCount: forcedResult.assignments.length,
      ...
      for (let period = 1; period <= siteContext.numSlots; period += 1) {
        ...
        slotDocuments.push({
          ...
          config: {
            ...
            numSlots: Number(siteContext.numSlots || 8),
...
function getAssignmentConstraintIssues(assignment) {
  return normalizeConstraintIssues(
    assignment?.constraintIssues ||
    assignment?.placement?.constraintIssues ||
    assignment?.task?.automaticPlanningWarnings ||
    []
  )
}

function buildAutomaticOverrideReason(issues = []) {
  const descriptions = normalizeConstraintIssues(issues)
    .map((issue) => issue.description)
    .filter(Boolean)

  return descriptions.length > 0
    ? descriptions.join(' ')
    : 'TPI placé automatiquement avec contrainte non respectée.'
}

function buildConflictDocumentsFromConstraintIssues(issues = []) {
  return normalizeConstraintIssues(issues).map((issue) => ({
    type: issue.type,
    description: issue.description,
    involvedPersons: issue.involvedPersons,
    detectedAt: new Date()
  }))
}
  ...
  return Array.from(groupedRooms.values())
    .map((room) => {
      ...
      const tpiDatas = Array.from({ length: maxPeriod }, (_, index) => {
        ...
        const constraintWarnings = normalizeConstraintIssues(entry?.task?.automaticPlanningWarnings || [])
          .map((issue) => ({
            type: issue.type,
        ...
        return {
          ...
          isConstraintOverride: constraintWarnings.length > 0,
          planningOverrideReason: constraintWarnings.map((issue) => issue.message).filter(Boolean).join(' '),
          constraintWarnings
        }
      })
...
async function autoPlanYear(year) {
  ...
  const [planningConfig, planningCatalog] = await Promise.all([
    getPlanningConfig(coordinationYear),
    getSharedPlanningCatalog()
  ])

  const rawTpis = await TpiPlanning.find(buildPlanifiableTpiQuery(coordinationYear))
    .populate('candidat')
    .populate('expert1')
    .populate('expert2')
    .populate('chefProjet')
  ...
  await Slot.deleteMany({ year: coordinationYear })
  ...
  const bulkOperations = tasks.map((task) => {
    ...
    if (assignment) {
      const constraintIssues = getAssignmentConstraintIssues(assignment)
      const hasConstraintOverride = constraintIssues.length > 0
        ...
        $set: {
          ...
          proposedSlots: slot
            ? [{
                slot: slot._id,
                proposedAt: new Date(),
                score: hasConstraintOverride ? 10 : 100,
                reason: hasConstraintOverride
                  ? `${reason} • Contrainte ignorée`
                  : reason
              }]
            : [],
          confirmedSlot: null,
          ...
          conflicts: hasConstraintOverride
            ? buildConflictDocumentsFromConstraintIssues(constraintIssues)
            : []
        },
        $unset: {
          votingSession: ''
      ...
      if (hasConstraintOverride) {
        update.$set.manualOverride = {
          isManual: true,
          reason: buildAutomaticOverrideReason(constraintIssues),
          overriddenAt: new Date()
        }
      } else {
        update.$unset.manualOverride = ''
      }
  ...
  const tasksWithConstraintWarnings = tasks.map((task) => {
    const assignment = assignmentsByTpiId.get(task.tpiId)
    const constraintIssues = getAssignmentConstraintIssues(assignment)

    return constraintIssues.length > 0
      ? {
          ...task,
          automaticPlanningWarnings: constraintIssues
        }
      : task
  })
  const legacyRooms = await syncLegacyRoomsFromAutomaticPlanning(coordinationYear, insertedSlots, tasksWithConstraintWarnings)
  ...
  return {
    year: coordinationYear,
    totalTpis: tasks.length,
    plannedCount: computation.assignments.length,
    manualRequiredCount: computation.manualRequired.length,
    constraintOverrideCount: computation.constraintOverrideCount || computation.assignments.filter((assignment) => getAssignmentConstraintIssues(assignment).length > 0).length,
