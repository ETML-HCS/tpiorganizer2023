...
test('generateStaticVotesSite writes PHP, sync endpoint and manifest in the vote folder', async (t) => {
  ...
  await withVoteEnv({
    ...
    try {
      ...
      assert.equal(result.archivedCampaignId, null)
      ...
      votes[0]._id = new mongoose.Types.ObjectId()
      const regenerated = await generateStaticVotesSite(2026)
      const archive = JSON.parse(fs.readFileSync(regenerated.campaignArchivePath, 'utf8'))
      assert.equal(Array.isArray(archive.campaigns), true)
      assert.equal(archive.campaigns[0].campaignId, result.campaignId)
      assert.equal(archive.campaigns[0].payload.groups.length, 1)
    } finally {
      while (restore.length > 0) {
        restore.pop()()
      }
...
test('buildStaticVotePhp gates the vote UI with token hashes and writes JSONL submissions', () => {
  ...
  assert.match(php, /staticVoteRecordMatchesCurrentGroup/)
  assert.match(php, /staticVoteFixedSlotKey/)
  assert.match(php, /staticVoteSubmittedTpiIds\(string \$tokenHash, string \$campaignId, array \$groups\)/)
  assert.match(php, /staticVoteSubmittedTpiIds\(\s*\$staticVoteTokenHash,\s*staticVoteText/)
  assert.match(php, /staticVoteFindExistingSubmission\(\$tokenHash, \$campaignId, \$tpiId, \$group\)/)
