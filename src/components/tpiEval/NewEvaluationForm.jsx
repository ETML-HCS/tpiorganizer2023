...
// Fonction pour afficher les données dans les éléments HTML appropriés
const displayData = (d) => {
  // nous avons un gros problème ici
  // l'injection des données n'affectent en rien les useStates comment faire pour les mettre aussi à jours ?

  if (d != null) {
    // Mise à jour des points dans les éléments HTML appropriés
    injectPoints("Asomme", d.pointsObtenusA)
    injectPoints("Bsomme", d.pointsObtenusB)
    injectPoints("Csomme", d.pointsObtenusC)
    injectPoints("Contraction_Asomme", d.pointsObtenusA)
    injectPoints("Contraction_Bsomme", d.pointsObtenusB)
    injectPoints("Contraction_Csomme", d.pointsObtenusC)
    injectPoints("Contraction_ABCsomme", d.pointsObtenusABC)
    injectPoints("Contraction_note", d.noteObtenu)

    // Affichage des données dans les éléments HTML appropriés
    updateElementValue("refCandidat", d.tpiRef)
    updateElementValue("remarque", d.tpiRemarque)
    populateElementsWithData("data-header", d.datasHeader)
    populateElementsWithData("data-justification", d.datasJustification)
    populateElementsWithData("data-ptechplus", d.dataPTechPlus)
    populateElementsWithData("data-ptechselected", d.dataPTechSelected)
    populateElementsWithData("data-point", d.dataPoints, 0)
  }
}
...
const populatePdfFields = async () => {
  try {
    ...
    // mappings entre le PDF et le DOM
    const fieldMappings = {
    ...
    for (const fieldName in fieldMappings) {
      ...
      // Construire le nom du fichier PDF en fonction des champs spécifiques
      if (fieldName === "headerCandidat1") {
        namePdf += fieldValue.replace(/\s+/g, "_") + "-"
      }

      if (fieldName === "nameCandidat") {
        namePdf +=
          fieldValue.replace(/\s+/g, "_") +
          "-" +
          new Date().toLocaleDateString("en-CA")
        // Remplacer les espaces par des tirets bas et ajouter l'année actuelle
      }

      // Modifier la taille de la police du champ PDF
      const fontSize = 8.5 // Taille de police souhaitée (en points)
    ...
    // Télécharger le PDF en utilisant un lien invisible
    const downloadLink = document.createElement("a")
    downloadLink.href = pdfUrl
    downloadLink.download = namePdf // Utiliser le nom du PDF que vous avez construit
    downloadLink.click()
...
function syncReactControlledField(element) {
  element.dispatchEvent(new Event("input", { bubbles: true }))
}

const populateElementsWithData = (attributeName, data, fallbackValue = "") => {
  const elementsWithData = document.querySelectorAll(`[${attributeName}]`)
  ...
  elementsWithData.forEach((element) => {
    ...
    if (element.tagName === "INPUT" || element.tagName === "TEXTAREA") {
      element.value = dataObject?.[attributeValue] ?? fallbackValue
      syncReactControlledField(element)
    } else {
...
const injectPoints = (elementId, value) => {
...
// Fonction pour mettre à jour la valeur d'un élément HTML s'il existe
function updateElementValue(elementName, dataValue) {
  const element = document.getElementsByName(elementName)[0]
  if (element) {
    element.value = dataValue ?? ""
    syncReactControlledField(element)
  } else {
    console.error("L'élément '" + elementName + "' n'existe pas.")
  }
}
...
function NewEvaluationForm({ searchCandidat, loadTpiEval, setLoadTpiEval, onEvaluationSaved, onEvaluationDeleted }) {
  ...
  async function saveTpiEvalLocally(evaluationData) {
    try {
      const savedEvaluations = upsertJSONListValue(
      ...
      const savedKey = getEvaluationStorageKey(evaluationData)
      const savedEvaluation =
        savedEvaluations.find((item) => getEvaluationStorageKey(item) === savedKey) ||
        savedEvaluations[savedEvaluations.length - 1] ||
        evaluationData
  ...
  const toggleFreeze = useCallback(() => {
    setIsFreeze((prevIsFreeze) => !prevIsFreeze)
  }, [])
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (loadTpiEval != null) {
        ...
        try {
          setData(loadTpiEval)
          setIsActive(Boolean(loadTpiEval?.p_appro))

          const loadedYear = Number(loadTpiEval?.year)
          if (Number.isFinite(loadedYear)) {
            setYearForThisEval(loadedYear)
          }

          setShowDownloadButton(true)
          setIsFreeze(true)
          const loadedPoints = [
            loadTpiEval?.pointsObtenusA,
            loadTpiEval?.pointsObtenusB,
            loadTpiEval?.pointsObtenusC,
            loadTpiEval?.pointsObtenusABC,
            loadTpiEval?.noteObtenu
          ]
          setResultsOfParts((prevResults) =>
            prevResults.map((result, index) => {
              const nextPoints = Number(loadedPoints[index])
              return {
                ...result,
                pointsObtenus: Number.isFinite(nextPoints) ? nextPoints : 0
              }
            })
          )
        } catch (error) {
  ...
  // Permet de basculer d'un état éditable ou non sans perdre les informations des champs de texte
  useEffect(() => {
    freezeFields(isFreeze)
    displayData(data)
  }, [data, freezeFields, isFreeze])
  ...
  const handleDownload = () => {
    if (data) {
      const candidateName = data.datasHeader["Candidat.eName"] // Nom du candidat extrait des données
      const truncatedName = candidateName.replace(/\s/g, "").substring(0, 10) // Limite à 10 caractères et suppression des espaces
      const tpiRef = data.tpiRef // Référencement TPI extrait des données

      // Formatage de la date au format YYYYMMDD
      ...
      a.href = url
      a.download = `eval${tpiRef}-${truncatedName}_${timestamp}.json` // Nom du fichier avec le nom du candidat, le référencement TPI et la date
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
    } else {
      console.error("Aucune donnée à télécharger.")
    }
  }

  // freezeFields et toggleFreeze sont définis plus haut via useCallback

  const handleFreeze = async () => {
    if (data !== null) {
      try {
        const savedEvaluation = await saveTpiEvalLocally(data)
        const pdfGenerated = await populatePdfFields()

        if (savedEvaluation) {
          setData(savedEvaluation)
        }
        ...
        if (!pdfGenerated) {
          toast.error(
            "Le PDF n'a pas pu être généré, mais l'évaluation a été enregistrée localement."
          )
        }
        // saveTpiEval(data); - pour la version finale ...
      } catch (error) {
        console.error("Erreur lors de la clôture de l'évaluation :", error)
        toast.error("Impossible de clôturer l'évaluation.")
      }
    } else {
      toast.info(
        "Le système a besoin d'une sauvegarde avant de faire un freeze"
      )
    }
  }
  ...
  const handleClear = () => {
    if (data != null) {
      ...
      // Réinitialisation des valeurs des champs utilisés dans la fonction handleSave
      const fieldsToReset = [
        "Asomme",
        "Bsomme",
        "Csomme",
      ...
      fieldsToReset.forEach((fieldName) => injectPoints(fieldName, 0))

      document.getElementsByName("refCandidat")[0].value = ""
      document.getElementsByName("remarque")[0].value = ""

      populateElementsWithData("data-header", null)
      populateElementsWithData("data-justification", null)
      populateElementsWithData("data-ptechplus", null)
      populateElementsWithData("data-ptechselected", null)
      populateElementsWithData("data-point", null, 0)
  ...
  const handleClickFreeze = () => {
    toast.warning(
      "Attention : Les champs peuvent être instables, pensez à sauvegarder fréquemment."
    )
    toggleFreeze()
  }

  const handleClickDeleteEval = (id) => {
    try {
      const existingEvaluations = readJSONListValue(
        STORAGE_KEYS.EVALUATION_DATA,
        [],
        ["evaluationData"]
      )
      ...
      if (existingEvaluations.length === 0) {
        toast.error("Aucune donnée d'évaluation trouvée localement")
        return
      }

      const nextEvaluations = removeFromJSONListValue(
        STORAGE_KEYS.EVALUATION_DATA,
        (item) => {
          const itemKey = getEvaluationStorageKey(item)
          return String(item?.id ?? itemKey) === String(id)
        },
        ["evaluationData"]
      )

      if (nextEvaluations.length === existingEvaluations.length) {
        toast.error(`Aucune évaluation trouvée pour l'ID ${id}`)
        return
      }
      ...
      toast.info(`L'évaluation avec l'ID ${id} a été supprimée`)
    } catch (error) {
      console.error(
        "Erreur lors de la suppression de l'évaluation localement :",
        error.message
      )
      throw error
    }
  ...
  return (
    <>
      <div className='toggleMode'>
        ...
        <span id='tpiEvalMode'>
          <div className='label-tpiEvaFreeze'>Évaluation Clôturée</div>
          <button
            type='button'
            className={`toggle-button ${isFreeze ? "active" : ""}`}
      ...
      <div
        className={`btn_validation ${
          showDownloadButton ? "enable" : "disable"
        }`}
      >
        <button
          type='button'
