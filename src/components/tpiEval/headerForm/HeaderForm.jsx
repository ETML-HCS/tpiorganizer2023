...
async function getExpertByName(name) {
  try {
    const response = await fetch(`${apiUrl}/api/expert/${encodeURIComponent(String(name || ''))}`);
    if (!response.ok) {
...
function showPopup(textHTML) {
  return new Promise((resolve, reject) => {
    ...
    if (!popup || !popupContent) {
      reject(new Error("Popup introuvable"));
      return;
    }
    ...
    const cleanup = () => {
      popupContent.removeEventListener("change", handleChange);
      if (popup.onclick === handleBackdropClick) {
        popup.onclick = null;
      }
    };
    ...
    popupContent.addEventListener("change", handleChange);
...
const insertDetails = async (c, refCandidat) => {
  if (c != null) {
    ...
    const refTpi = document.querySelectorAll('[name="refCandidat"]');
    ...
    expert1NameInput.value = c.experts["1"];
    const expert1 = await getExpertByName(c?.experts["1"]);
    expert1EmailInput.value = expert1 ? expert1.email : "";

    // Insérer les détails du deuxième expert
    expert2NameInput.value = c.experts["2"];
    const expert2 = await getExpertByName(c.experts["2"]);
    expert2EmailInput.value = expert2 ? expert2.email : "";

    remarque.value = "Sujet : " + c.sujet;

    refTpi.forEach((element) => {
      element.value = c.refTpi;
    });

    // Nécessité d'appliquer cette opération pour supprimer le focus causé par le bouton de recherche

    var temp = c.refTpi + "-" + c.candidat;
    var truncatedText = temp.substring(0, 18);
    refCandidat(truncatedText);
...
const HeaderLine = ({ isVisible, searchCandidat }) => {
            ...
            showPopup(candidats)
              .then((selectedCandidateName) => {
                ...
                if (selectedCandidate) {
                  ...
                  console.error(
                    "Candidat non trouvé dans la liste des candidats"
                  );
                }
              })
              .catch((error) => {
  ...
  const handleSearchClick = () => {
    ...
    if (year.trim() === "") {
      console.error("L'année est vide");
      return; // Arrêter l'exécution si l'année est vide
    }

    // Convertir l'année en un nombre entier
    const yearInt = parseInt(year, 10);

    // Vérifier si l'année est un nombre valide
    if (isNaN(yearInt) || yearInt < 1900 || yearInt > 2100) {
      console.error("L'année est invalide");
      return; // Arrêter l'exécution si l'année est invalide
    }

    // Appeler la fonction de recherche avec l'année et le candidat à rechercher
    const matches = searchCandidat(yearInt, searchThisCandidat);

    // Mettre à jour l'état local avec les résultats de la recherche
    setTpiMatches(matches);
  };
