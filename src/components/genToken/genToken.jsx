...
const TokenGenerator = () => {
  ...
  const handleGenerateTokens = async () => {
    ...
    for (const { email, name } of experts) {
      // Déstructuration pour obtenir email et name
    ...
    try {
      const response = await axios.put(
        `${apiUrl}/api/experts/putTokens`,
        update
      )
    } catch (error) {
      console.error(
        `Erreur lors de la sauvegarde dans la base de données pour ${update} :`,
        error
      )
    }
  ...
  return (
    <div className='container' style={{ marginTop: '30px' }}>
      ...
      <div className='form-group'>
        <button className='btn btn-primary' onClick={handleGenerateTokens}>
          Générer Tokens
        </button>
      </div>
