...
describe("planningCapacityUtils", () => {
  ...
  it("calculates room needs for a catalog site with repeated stakeholders", () => {
    ...
    expect(overview.totals.operationalRooms).toBe(2)
    expect(overview.totals.recommendedRooms).toBe(3)
    expect(overview.totals.roomGap).toBe(1)
    expect(overview.totals.shortageRooms).toBe(1)
    expect(overview.totals.surplusRooms).toBe(0)
    ...
    expect(overview.sites[0].pressureLabel).toBe("Forte")
  ...
  it("sizes on the busiest day when confirmed soutenances fall on the same date", () => {
    const overview = buildPlanningRoomSizingOverview({
    ...
    expect(overview.sites[0].theoreticalRooms).toBe(2)
    expect(overview.sites[0].recommendedRooms).toBe(2)
    expect(overview.sites[0].roomGap).toBe(1)
  })
